...
class Engine:
    def __init__(self, model, tok):
        ...
        self.scheduler_obj = Scheduler(self.block_manager_obj, core_configurations['block_size'], core_configurations['max_len'], skip_threshold=core_configurations['scheduler_skip_threshold'], lookahead=core_configurations['scheduler_lookahead'])
    ...
    def run(self):
        ...
        while True:
            ...
            logits = self.model(flat_tokens, self.block_manager_obj.block_table, length, kv_len_per_seq, sequence_id, offset, length, position_ids, pos_seq_id) # adding sequence_id as well because we need it for computing the num_blocks_per_seq in attention module by making sure that the sequence ids are exactly in order
            ...
            current_index = 0
            for seq in prefill_seq:
                if new_tokens[current_index] == self.tok.EOS_ID:
